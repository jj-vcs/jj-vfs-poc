...
fn load_commit_cloud_workspace(settings: &UserSettings, workspace_root: &Path) -> Workspace {
    ...
    store_factories.add_op_store(
        CommitCloudOpStore::name(),
        Box::new(|_settings, store_path, _root_op_id| {
            let config_path = get_config_path(store_path);
            let config_str =
                fs::read_to_string(&config_path).map_err(|e| BackendLoadError(e.into()))?;
            let (server_url, repo_id) =
                parse_config(&config_str).map_err(|e| BackendLoadError(e.into()))?;
