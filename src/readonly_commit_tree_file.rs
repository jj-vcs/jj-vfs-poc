...
pub struct ReadonlyCommitTreeFile {
    commit: Commit,
    path: PathBuf,
}

impl ReadonlyCommitTreeFile {
...
#[async_trait]
impl VirtualFile for ReadonlyCommitTreeFile {
...
mod tests {
    ...
    async fn test_list_directory_root() {
        ...
        let commit_tree = ReadonlyCommitTreeFile::new(repo.as_ref(), commit_id, PathBuf::from(""))
            .await
            .unwrap();
    ...
    async fn test_list_directory_sub() {
        let (_temp_dir, repo, commit_id) = setup_test_repo().await;
        let commit_tree =
            ReadonlyCommitTreeFile::new(repo.as_ref(), commit_id, PathBuf::from("dir"))
                .await
                .unwrap();
    ...
    async fn test_file_size() {
        let (_temp_dir, repo, commit_id) = setup_test_repo().await;
        let commit_tree =
            ReadonlyCommitTreeFile::new(repo.as_ref(), commit_id, PathBuf::from("file1.txt"))
                .await
                .unwrap();
    ...
    async fn test_invalid_path() {
        ...
        let commit_tree = ReadonlyCommitTreeFile::new(
            repo.as_ref(),
            commit_id.clone(),
        ...
        let commit_tree =
            ReadonlyCommitTreeFile::new(repo.as_ref(), commit_id, PathBuf::from("some/../path"))
                .await
                .unwrap();
    ...
    async fn test_file_type() {
        ...
        let commit_tree_dir =
            ReadonlyCommitTreeFile::new(repo.as_ref(), commit_id.clone(), PathBuf::from("dir"))
                .await
                .unwrap();
        let type_dir = commit_tree_dir.file_type().await.unwrap();
        assert!(matches!(type_dir, FileType::Directory));

        let commit_tree_file = ReadonlyCommitTreeFile::new(
            repo.as_ref(),
            commit_id.clone(),
        ...
        let commit_tree_symlink =
            ReadonlyCommitTreeFile::new(repo.as_ref(), commit_id, PathBuf::from("symlink"))
                .await
                .unwrap();
