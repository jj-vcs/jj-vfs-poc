...
trait JjFilesystem {
    async fn lookup(&self, parent: INodeNo, name: &str) -> Result<FileAttr, JjError>;
    async fn getattr(&self, ino: INodeNo) -> Result<FileAttr, JjError>;
    async fn read(&self, ino: INodeNo, offset: u64, size: u32) -> Result<Box<[u8]>, JjError>;
    async fn readdir<F>(&self, ino: INodeNo, offset: u64, f: F) -> Result<(), JjError>
    where
        Self: Sized,
        F: FnMut((INodeNo, DirectoryEntry)) -> bool;
}
...
impl<P: PathMapper> JjFilesystem for JjVfs<P> {
    async fn lookup(&self, parent: INodeNo, name: &str) -> Result<FileAttr, JjError> {
        let ino = self.inodes.get_ino(parent.0, name).await?;
        self.getattr(INodeNo(ino)).await
    }

    async fn getattr(&self, ino: INodeNo) -> Result<FileAttr, JjError> {
        let path = self.inodes.get_path(ino.0).await?;
    ...
    async fn readdir<F>(&self, ino: INodeNo, offset: u64, mut f: F) -> Result<(), JjError>
    where
        F: FnMut((INodeNo, DirectoryEntry)) -> bool,
    {
        if offset < 2 {
            ...
            if f((
                INodeNo(self.inodes.get_parent_ino(ino.0).await?),
        ...
        while let Some(entry) = skipped_stream.next().await {
            ...
            let child_ino = self.inodes.get_ino(ino.0, name).await?;
            if f((INodeNo(child_ino), DirectoryEntry::new(name, file_type))) {
                break;
            }
        }
...
mod tests {
    ...
    fn setup_test_vfs() -> JjVfs<MockPathMapper> {
    ...
    fn test_read_root_file() {
        ...
        let attr = futures::executor::block_on(fs.lookup(INodeNo(1), "file.txt")).unwrap();
        let content = futures::executor::block_on(fs.read(attr.ino, 0, 11)).unwrap();
    ...
    fn test_read_nested_file() {
        let fs = setup_test_vfs();
        let dir_attr = futures::executor::block_on(fs.lookup(INodeNo(1), "dir")).unwrap();
        let file_attr = futures::executor::block_on(fs.lookup(dir_attr.ino, "nested.txt")).unwrap();
        let content = futures::executor::block_on(fs.read(file_attr.ino, 0, 14)).unwrap();
    ...
    fn test_list_directory() {
        ...
        futures::executor::block_on(fs.readdir(dir_attr.ino, 0, |(child_ino, entry)| {
    ...
    fn test_list_directory_with_offset() {
        ...
        let mut entries = Vec::new();
        futures::executor::block_on(fs.readdir(dir_attr.ino, 2, |(child_ino, entry)| {
            entries.push((child_ino, entry.name, entry.file_type));
            false
        }))
        .unwrap();
